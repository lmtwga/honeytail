...
func (p *Parser) Init(options interface{}) error {
	...
	lineParser, err := NewCSVLineParser(p.conf.Fields)
	if err != nil {
		return err
	}
...
func NewCSVLineParser(fieldsString string) (*CSVLineParser, error) {
	...
	return &CSVLineParser{fields}, nil
}
...
func (p *CSVLineParser) ParseLine(line string) (map[string]interface{}, error) {
	...
	lenFields := len(p.fields)

	if lenValues != lenFields {
		logrus.WithFields(logrus.Fields{
			"num_values": lenValues,
			"num_fields": lenFields,
			"line":       line,
		}).Warn("number of values does not match provided fields")
	}

	for i := 0; i < int(math.Min(float64(lenValues), float64(lenFields))); i++ {
